...
describes what users get. Pick a format with `-o`, or let dadjoke choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dadjoke <command> -o table   # aligned columns for reading
dadjoke <command> -o jsonl   # one JSON object per line, for piping
dadjoke <command> -o json    # a single JSON array
dadjoke <command> -o csv     # spreadsheet friendly
dadjoke <command> -o tsv     # tab-separated
dadjoke <command> -o url     # just the URL column
dadjoke <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dadjoke <command> --fields id,title,url
```
...
```bash
dadjoke <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dadjoke <command>            # a table, because this is a terminal
dadjoke <command> | wc -l    # JSONL, because this is a pipe
```
